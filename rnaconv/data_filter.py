...
def filter_data(ali_dirpath, seq_dirpath, neigh_dbn_dirpath, neigh_ct_dirpath, max_dbrs_deviation=20):
	...
	consensus structure. If this results in leaving less than 2 sequences in the alignment, essentially deleting it or
	turning it into a single sequence, the whole alignment file with the corresponding consensus structure and sequence
	is removed.
	...
	for ali_filename in os.listdir(ali_dirpath):
		...
		if len(ali) <= 2:  # 1 would be only the CLUSTAL flag and therefore 0 sequences, 2 would be only 1 sequence
